...
import { BrandMark, PartnerCrest, LivePulse, VoiceOrb, Waveform, MelSpectrogram, EmbeddingCloud, SimilarityGauge, ConceptBars, PipelineFlow } from "./visuals.jsx";
...
// =============================================================================
// 6. TCAV EXPLANATION SCREEN
// =============================================================================
function ExplainScreen({ onRestart, name = 'Eden' }) {
  const concepts = useMemo(() => [
    { label: 'Long, rising pitch',   tech: 'long_rising_steep_thick',  score: 0.42 },
    { label: 'Steady, sustained tones', tech: 'long_constant_flat_thick', score: 0.31 },
    { label: 'Short, dropping notes', tech: 'short_dropping_steep_thin', score: 0.18 },
    { label: 'Broad energy spread',  tech: 'long_constant_flat_thick',  score: 0.14 },
    { label: 'Quick rising bursts',  tech: 'short_rising_steep_thin',   score: -0.06 },
    { label: 'Thin descending lines', tech: 'short_dropping_flat_thin', score: -0.11 },
  ], []);

  return (
    <div className="screen fade-enter">
      <Chrome status="EXPLAINABILITY · TCAV" statusKind="good" screenName="05 TCAV · EXPLAIN"/>

      <div style={{ position: 'absolute', inset: 0, padding: '140px 100px 130px', display: 'grid', gridTemplateColumns: '720px 1fr', gap: 80 }}>

        {/* Left */}
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 28 }}>
          <div>
            <div className="label-mono" style={{ color: 'var(--teal-2)', marginBottom: 14 }}>STEP 5 · WHY</div>
            <div style={{ fontSize: 76, fontWeight: 200, lineHeight: 1.02, letterSpacing: '-0.02em' }}>
              <span className="serif" style={{ fontStyle: 'italic', color: 'var(--teal-2)' }}>The shapes</span>
              <br/>
              <span style={{ color: 'var(--ink)' }}>that say it's you.</span>
            </div>
            <div style={{ marginTop: 22, fontSize: 19, color: 'var(--ink-mute)', maxWidth: 600, lineHeight: 1.5 }}>
              Most AI is a black box. BioVoice isn't.
              We surface the acoustic patterns the model leaned on — pitch contours,
              sustained tones, how energy is distributed — so a human reviewer can sanity-check
              every decision.
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 14 }}>
            <div className="panel" style={{ flex: 1, padding: '16px 20px' }}>
              <div className="label-mono" style={{ fontSize: 9 }}>METHOD</div>
              <div style={{ fontSize: 18, marginTop: 4, color: 'var(--ink)' }}>TCAV</div>
              <div className="num-mono" style={{ fontSize: 10, color: 'var(--ink-soft)', marginTop: 2 }}>
                Concept Activation Vectors
              </div>
            </div>
            <div className="panel" style={{ flex: 1, padding: '16px 20px' }}>
              <div className="label-mono" style={{ fontSize: 9 }}>LAYER</div>
              <div style={{ fontSize: 18, marginTop: 4, color: 'var(--teal-2)' }}>STAGE-4</div>
              <div className="num-mono" style={{ fontSize: 10, color: 'var(--ink-soft)', marginTop: 2 }}>
                ARI 0.48 · Silhouette 0.40
              </div>
            </div>
            <div className="panel" style={{ flex: 1, padding: '16px 20px' }}>
              <div className="label-mono" style={{ fontSize: 9 }}>OVERHEAD</div>
              <div style={{ fontSize: 18, marginTop: 4, color: 'var(--ink)' }}>+ 70 ms</div>
              <div className="num-mono" style={{ fontSize: 10, color: 'var(--ink-soft)', marginTop: 2 }}>
                On-demand only
              </div>
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 18 }}>
          <div className="panel outline-glow" style={{ padding: '36px 44px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 24 }}>
              <div>
                <div className="label-mono" style={{ fontSize: 10 }}>TOP CONTRIBUTING VOICE PATTERNS</div>
                <div style={{ fontSize: 22, marginTop: 6 }}>What helped identify <span className="serif" style={{ color: 'var(--teal-2)', fontStyle: 'italic' }}>{name}</span></div>
              </div>
              <div className="label-mono" style={{ fontSize: 9, color: 'var(--ink-soft)' }}>POSITIVE = SUPPORTS · NEGATIVE = OPPOSES</div>
            </div>
          ...
          <div className="panel" style={{ padding: '18px 24px' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
              ...
              <div style={{ fontSize: 14, color: 'var(--ink-mute)', lineHeight: 1.6 }}>
                <span style={{ color: 'var(--ink)' }}>Plain reading: </span>
                {name}'s voice is recognized mostly by long, rising pitch shapes and steady sustained tones —
                a vocal signature consistent across this and prior enrollments. Every score above is auditable.
              </div>
            </div>
          </div>
        </div>
