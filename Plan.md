...
> **Owner of this file:** Eden (keep updated as phases progress)
> **Source of truth for screens:** `SDD-6 Riva.pdf` §5 (Figures 15–20). **TCAV (Figure 19) is dropped** for this milestone — it is not yet working and is removed from scope until further notice.
...
| # | Screen (SDD §) | Mockup |
|---|---|---|
| 1 | Enrollment Screen (5.1) | Fig. 15 |
| 2 | Processing Screen (5.2) | Fig. 16 |
| 3 | Deepfake Detection Result (5.3) | Fig. 17 |
| 4 | Verification Result (5.4) | Fig. 18 |
| 5 | Deepfake Generator / Test Lab (5.6) | Fig. 20 |
...
4. **Backend additions are minimal.** We extend, not rewrite, the existing FastAPI services. New endpoints:
   - `GET /users/{user_id}/availability` — returns `{ available: bool }` for the enrollment ID-Available pill.
   - `POST /me/spoof/test` — runs the AASIST detector on an in-flight spoof sample and returns `{ deepfake_score, decision }` so the Test Lab "Test Detection" button can show a result without enrolling/verifying anyone.
   - `GET /me/verifications/{result_id}` — returns the full verification record (including stage breakdown the result screen displays). Keeps the Verification Result screen self-contained.
...
Each phase is a logical, independently testable chunk. Phase numbers map to assigned tasks in `Eden.md`, `Idan.md`, `Yoav.md`.
...
**Owner:** Idan (backend audit) + Eden (frontend audit).

### Phase 1 — Design system foundation ⬜

- New `tokens.css` with light-theme CSS variables matching the mockups (off-white backgrounds, black text, semantic greens/reds/blues/purples for status pills, the orange "test mode" warning band, the violet "what does this mean" callout if it ever returns, etc.).
...
**Owner:** Eden.
**Exit:** Storybook-style demo route (or a `?showcase=1` query) that renders one of each primitive, signed off by all three engineers before screens consume the primitives.
...
**Owners:** Yoav (screen) + Idan (availability endpoint).
...
**Owner:** Yoav (UI) + Idan (deepfake sub-score derivation lives in `detector.py`).
...
**Owners:** Eden (screen) + Idan (`/me/verifications/{result_id}` endpoint, session ID format).
...
**Owners:** Yoav (UI) + Idan (`/me/spoof/test` endpoint).
...
|---|---|---|---|
| `/users/{user_id}/availability` | GET | ID-Available pill | Returns `{ available, suggestions: [] }`. No auth. |
| `/me/verifications/{result_id}` | GET | View Details modal | Auth required. 404 if not the user's record. |
| `/me/spoof/test` | POST | AASIST score for an arbitrary WAV | Auth required. Multipart `audio`. Returns `{ deepfake_score, decision }`. |

Plus:

- `verification.py` already returns `centroid_similarity` and `sample_similarities`. Add `session_id` (formatted) and a `stage_breakdown: {load_ms, resample_ms, mel_ms, embed_ms, detect_ms}` to `VerificationResponse`. Wire timings with `time.perf_counter()` around the existing call sites.
- `detector.py` extends to expose `analysis_details: { voice_naturalness, spectral_consistency, temporal_patterns, artifact_detection }`. For now derive deterministically from the global AASIST score with seeded jitter; document in the paper.
...
- Cross-engineer code review (`gh pr` workflow on the `web-refactor-plan` style branch).

**Owners:** All three. Eden coordinates.

## 5. Repository layout after the rebuild
...
backend/app/
  ...
  services/
    ...
    verification.py        # + stage_breakdown, + session_id
```
...
| AASIST sub-scores are not real per-metric outputs. | Deterministic seeded derivation, transparently documented in the research paper appendix. Idan owns the derivation function and the documentation paragraph. |
...
| < 2 s latency budget on cold ReDimNet load. | Warm the model on backend startup (already done in `services/speaker_encoder.py` if `lazy=False`). Verify in Phase 9. |
...
- Weekly sync: 30 min, all three engineers, Wednesday 18:00 IDT.
- Slack-equivalent updates in this `Plan.md` checkbox grid (mark phase status as you go).
- PRs target `main` via `web-refactor-plan` style topic branches, one per phase.
- Code review: at least one of the other two engineers must approve. UI work additionally needs a screenshot in the PR description.

## 8. Open questions

- [ ] Confirm: do we want `Yoav.md` (matches SDD team) or a different filename? (Currently `Yoav.md`.)
